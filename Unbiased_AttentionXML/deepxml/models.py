...
class Model(object):
    ...
    """
    def __init__(self, network, model_path, per_cls_weights, gradient_clip_value=5.0, device_ids=None, **kwargs):
    ...
    def train_step(self, train_x: torch.Tensor, train_y: torch.Tensor):
        ...
        self.loss_fn = nn.BCEWithLogitsLoss(pos_weight=per_cls_weights)
    ...
    @staticmethod
    def get_weights(reweighting, cls_num_list, a, b, c, beta):
        per_cls_weights = None
        num_labels = len(cls_num_list)
        inv_prop = 1 + c * torch.pow((cls_num_list+b), -a)
        if reweighting == 'PW':
...
class XMLModel(Model):
    ...
    """
    def __init__(self, labels_num, hidden_size, per_cls_weights, device_ids=None, attn_device_ids=None,
        ...
        super(XMLModel, self).__init__(hidden_size=hidden_size, device_ids=device_ids, labels_num=labels_num,
                                       parallel_attn=parallel_attn, per_cls_weights=per_cls_weights, **kwargs)
    ...
    def train_step(self, train_x: Tuple[torch.Tensor, torch.Tensor], train_y: torch.Tensor):
        ...
        per_cls_weights = self.per_cls_weights[candidates] if self.per_cls_weights is not None else None
