...
@click.option('--reweighting', type=click.Choice(['PW', 'PW-cb', 'None']), default=None)
...
@click.option('--beta', type=click.FLOAT, default=0.9, help='Hyperparameter of class balanced term')
@click.option('-t', '--tree-id', type=click.INT, default=None)

def main(data_cnf, model_cnf, mode, tree_id, reweighting, a, b, beta):
    ...
    reweighting = 'None' if reweighting is None else reweighting
    model_path = F'{model_path}-reweighting-{reweighting}{tree_id}'
    ...
    if mode is None or mode == 'train':
        ...
        if 'cluster' not in model_cnf:
            model_cnf['train']['nb_epoch'] = int(model_cnf['train']['nb_epoch']*1.2) 
            ...
            # computing the weighting factors
            cls_num_list = torch.FloatTensor(np.sum(train_y,axis=0))[0].cuda() 
            cls_num_list[cls_num_list==0] = 1.0
            c = (np.log(train_x.shape[0])-1)*np.power(b+1, a)
            per_cls_weights = Model.get_weights(reweighting, cls_num_list, a, b, c, beta)
            model = Model(network=AttentionRNN, labels_num=labels_num, model_path=model_path, per_cls_weights=per_cls_weights, 
                          emb_init=emb_init, **data_cnf['model'], **model_cnf['model'])
            model.train(train_loader, valid_loader, **model_cnf['train'])
        else:
            model = FastAttentionXML(labels_num, data_cnf, model_cnf, a ,b, beta, reweighting, tree_id)
    ...
    if mode is None or mode == 'eval':
        ...
        if 'cluster' not in model_cnf:
            test_loader = DataLoader(MultiLabelDataset(test_x), model_cnf['predict']['batch_size'],
                                     num_workers=4)
            if model is None:
                model = Model(network=AttentionRNN, labels_num=labels_num, model_path=model_path, reweighting = reweighting, per_cls_weights = per_cls_weights, cls_num_list = cls_num_list, emb_init=emb_init,
                              **data_cnf['model'], **model_cnf['model'])
            scores, labels = model.predict(test_loader, k=model_cnf['predict'].get('k', 100))
        else:
            if model is None:
                model = FastAttentionXML(labels_num, data_cnf, model_cnf, cls_num_list =None , a=0.55 ,b=1.5, reweighting = reweighting, tree_id=tree_id)
            scores, labels = model.predict(test_x)
        logger.info('Finish Predicting')
        labels = mlb.classes_[labels]
        output_res(data_cnf['output']['res'], F'{model_name}-{data_name}-reweighting-{reweighting}{tree_id}', scores, labels)
