...
class FastAttentionXML(object):
    ...
    """
    def __init__(self, labels_num, data_cnf, model_cnf, a, b, beta, reweighting, tree_id=''):
        self.data_cnf, self.model_cnf = data_cnf.copy(), model_cnf.copy()
        model_name, data_name = model_cnf['name'], data_cnf['name']
        self.model_path = os.path.join(model_cnf['path'], F'{model_name}-{data_name}-reweighting-{reweighting}{tree_id}')
    ...
    def train_level(self, level, train_x, train_y, valid_x, valid_y):
        c = (np.log(train_x.shape[0])-1)*np.power(self.b+1, self.a)
        ...
        if level == 0:
            ...
            per_cls_weights = Model.get_weights(self.reweighting, cls_num_list, self.a, self.b, c, self.beta)

            model = Model(AttentionRNN, per_cls_weights=per_cls_weights, labels_num=labels_num, model_path=F'{self.model_path}-Level-{level}',
            ...
            if level < self.level - 1:
                ...
                cls_num_list = torch.FloatTensor(np.sum(train_y,axis=0))[0].cuda() # mohamm
                cls_num_list[cls_num_list==0] = 1.0
                per_cls_weights = Model.get_weights(self.reweighting, cls_num_list, self.a, self.b, c, self.beta)
            else:
            ...
            valid_loader = DataLoader(XMLDataset(valid_x, valid_y, training=False, labels_num=labels_num,
                                      ...
                                      model_cnf['valid']['batch_size'], num_workers=4)
            model = XMLModel(network=FastAttentionRNN, per_cls_weights=per_cls_weights, labels_num=labels_num, emb_init=self.emb_init,
    ...
    def predict_level(self, level, test_x, k, labels_num):
        ...
        if level == 0:
            ...
            if model is None:
                model = Model(AttentionRNN, labels_num=labels_num, model_path=F'{self.model_path}-Level-{level}', per_cls_weights = None,reweighting = self.reweighting,
                              emb_init=self.emb_init, **data_cnf['model'], **model_cnf['model'])
            ...
            if model is None:
                model = XMLModel(network=FastAttentionRNN, labels_num=labels_num,
                                 model_path=F'{self.model_path}-Level-{level}', per_cls_weights = None, reweighting = self.reweighting,
                                 emb_init=self.emb_init, **data_cnf['model'], **model_cnf['model'])
    ...
    def train(self, train_x, train_y, valid_x, valid_y, mlb):
        ...
        build_tree_by_level(self.data_cnf['train']['sparse'], self.data_cnf['train']['labels'], mlb, **self.model_cnf['cluster'])
