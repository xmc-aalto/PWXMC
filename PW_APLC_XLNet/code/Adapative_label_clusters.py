...
class AdaptiveBCEWithLogitsLoss(Module):
    ...
    * :attr:`head_bias` if set to True, adds a bias term to the 'head' of the
    ...
    def forward(self, input, target, reweighting_factors):
        ...
        for i in range(len(cutoff_values) - 1):
            ...
            if i == 0:
                ...
                loss = AdaptiveBCEWithLogitsLoss.weighted_binary_cross_entropy(
                    cluster_output.view(-1, num_idx), target_onehot.view(-1, num_idx).float(),
                    pos_weights=reweighting_factors[i].to('cuda') if reweighting_factors[i] is not None else None)
