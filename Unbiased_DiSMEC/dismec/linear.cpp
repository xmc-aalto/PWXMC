...
//		train_one(prob, param, &model_->w[0], 0, 0);
	...
	else
	{
		...
		long long nr_class = param->num_class; // @mohamm
...
//			printf("class Count is class %d count = %d\n", i, classCount[i]);
		...
		//////////////////// @mohamm
                double A = param->ap;
                double B = param->bp;
                double C = (log(l)-1)*pow(B+1,A);
		double beta = 0.9;
                double *weighted_C = Malloc(double, nr_class);
		if(param->reweighting_type == PW){
			for(i=0;i<nr_class;i++){
                        	if(classCount[i]>0){
					double inv_p = 1 + C*pow(classCount[i]+B,-A);
                                	weighted_C[i] = 2 * inv_p - 1;
                        	}
                        	else
                                	weighted_C[i] = param->C;
			}		
		}
		else if(param->reweighting_type == PW_cb){
			for(i=0;i<nr_class;i++){
                        	if(classCount[i]>0){
					double cb = (1-beta)/(1-pow(beta,classCount[i]));
					double inv_p = 1 + C*pow(classCount[i]+B,-A);
                                	weighted_C[i] = cb  * (2*inv_p-1);
                        	}
                        	else
                                	weighted_C[i] = param->C;
                	}		
		...
		else
		{
			...
			else
			{
				...
				int batch = param->i;
				int batchSize = param->batch_size; // @mohamm
				info("batch is %d\n", batch);
				if(batch == 0){
					startInd = 0;
					endInd = nr_class;
